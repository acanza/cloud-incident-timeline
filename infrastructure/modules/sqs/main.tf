...
# Dead-letter queue for failed timeline messages
resource "aws_sqs_queue" "timeline_dlq" {
  name                      = "${var.project_name}-${var.environment}-timeline-dlq"
  message_retention_seconds = 1209600 # 14 days
  tags                      = var.tags
}

# Main timeline queue with redrive policy pointing to DLQ
resource "aws_sqs_queue" "timeline_queue" {
  name                       = "${var.project_name}-${var.environment}-timeline-queue"
